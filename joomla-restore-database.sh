#!/usr/bin/env bash
# joomla-restore-database.sh [backup-file-name]
#
# Replaces the Joomla database with one of the dumps the backups service wrote.
#
#   ./joomla-restore-database.sh               list and ask
#   ./joomla-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than joomla.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-joomla}"
APP_SERVICE="joomla"
...
DB_NAME="$(env_of JOOMLA_DB_NAME)"; DB_USER="$(env_of JOOMLA_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Joomla." >&2
  exit 1
fi
