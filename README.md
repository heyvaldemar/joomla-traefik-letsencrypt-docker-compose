# Joomla + Traefik + Let's Encrypt on Docker Compose
...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14874/badge)](https://www.bestpractices.dev/projects/14874)
...
Every release from v1.8.7 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `joomla-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `joomla-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `joomla-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob joomla-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle joomla-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/joomla-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact joomla-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path joomla-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/joomla-traefik-letsencrypt-docker-compose
```
