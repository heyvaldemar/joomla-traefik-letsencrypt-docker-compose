#!/usr/bin/env bash
# joomla-restore-application-data.sh [backup-file-name]
#
# Replaces Joomla's application data with one of the archives the backups
# service wrote.
#
#   ./joomla-restore-application-data.sh               list and ask
#   ./joomla-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Joomla." >&2
  exit 1
fi
